...
export async function createDistribution(params: CreateDistributionRequest): Promise<CreateDistributionResponse> {
  const response = await fetch(`${getBaseUrl()}/api/distributions`, {
    method: 'POST',
    headers: {
...
export async function enableDistribution(id: string): Promise<UpdateDistributionResponse> {
  const response = await fetch(`${getBaseUrl()}/api/distributions/${id}/enable`, {
    method: 'POST',
  });
  return response.json();
}
...
export async function disableDistribution(id: string): Promise<UpdateDistributionResponse> {
  const response = await fetch(`${getBaseUrl()}/api/distributions/${id}/disable`, {
    method: 'POST',
  });
  return response.json();
}

export async function updateComments(id: string, comments: string): Promise<UpdateDistributionResponse> {
  const response = await fetch(`${getBaseUrl()}/api/distributions/${id}/comments`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
